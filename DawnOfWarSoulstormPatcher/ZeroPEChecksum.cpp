...
bool ZeroPEChecksum(const std::string& exePath) {
    Log_Write(LOG_INFO, "ZeroPEChecksum", "Opening file %s", exePath.c_str());

    std::fstream f(exePath, std::ios::in | std::ios::out | std::ios::binary);
    if (!f.is_open()) {
        Log_Write(LOG_ERROR, "ZeroPEChecksum", "Failed to open file.");
        return false;
    }
    ...
    if (dosH.e_magic != IMAGE_DOS_SIGNATURE) {
        Log_Write(LOG_ERROR, "ZeroPEChecksum", "Invalid DOS signature.");
        f.close();
        return false;
    }
    ...
    if (nth.Signature != IMAGE_NT_SIGNATURE ||
        ...
        Log_Write(LOG_ERROR, "ZeroPEChecksum", "Invalid NT header.");
        f.close();
        return false;
    }
    ...
    Log_Write(LOG_INFO, "ZeroPEChecksum", "Succeeded.");
    return true;
}
